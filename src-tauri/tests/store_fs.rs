...
//!
//! These run without a Tauri runtime: the store's core takes a `Weak` to itself
//! and an `EventSink`, neither of which is a Tauri type, so `cargo test` drives
//! the whole of it. Emissions go to a `RecordingSink` and are counted exactly —
//! "emits exactly one `settings-changed`" is the kind of claim that cannot be
//! verified by reading a thin command wrapper (A9.37).
//!
...
fn a_callback_for_a_closed_space_is_inert() {
	...
	copper_lib::store::watch::handle_external_change(&harness.shared, &stale_path);
